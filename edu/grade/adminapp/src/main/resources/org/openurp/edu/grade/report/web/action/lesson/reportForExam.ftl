...
[#list courseStats as courseStat]
  ...
  [#if courseStat.gradeSegStats.size()>0]
	[@b.div style="text-align:center"]
		<h2>[@i18nName teachTask.project.school!/]课程考核试卷分析表</h2>
		(${teachTask.semester.schoolYear!}学年 &nbsp;${(teachTask.semester.name)?if_exists?replace("1","第一")?replace("2","第二")}学期)
		<br/><br/>
	[/@]
 	 <table align="center" width="95%" border='0' style="font-weight:bold;">
 	 	<tr>
      <td>开课院(系、部):[@i18nName teachTask.teachDepart/]</td>
      <td>授课班级:[@i18nName teachTask.teachClass/]</td>
  ...
  [#else]
	   	...
	   	[@b.div style="width:95%;text-align:left;margin-top:20px;color:red;"]
	   		${teachTask.semester.schoolYear}学年${teachTask.semester.name}学期,${teachTask.teachClass.name!}，${teachTask.course.name} 没有出卷考试，无需打印填写试卷分析表。
	   	[/@]
	  	</td></tr></table>
  [/#if]	
[/#list]
