...
[#list clazzes as clazz]
    ...
    [#else]
       [#--全部重修--]
       [#assign squadMap = {allCourseTakers?first.std.squad.name:allCourseTakers?first.std.squad}/]
       [#assign squadCourseTakers = {allCourseTakers?first.std.squad.name:allCourseTakers}/]
       [#assign squads = squadMap?keys/]
    [/#if]
